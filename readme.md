...
Further addition: Script now supports a mechanism to retrieve classification for funds/ETFs from an alternative ISIN. It is used when Morningstar data for the native ISIN does not contain some classification data for a taxonomy. User needs to add #PPC:[ISIN2=*XY0011223344*] with the desired ISIN value to note field of the security in PP (besides other content). Script eill then try to retrieve missing information form this alternative ISIN. (This does not work for individual stocks, but only for funds/ETFs).
